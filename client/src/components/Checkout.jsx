...
function Checkout() {
  ...
  async function fetchingAddress() {
    const response = await axios.get("https://dogandcat-production.up.railway.app/api/address", {
  ...
  async function fetchingCartItems() {
    const response = await axios.get("https://dogandcat-production.up.railway.app/api/cart", {
      headers: { Authorization: `Bearer ${token}` },
    });
  ...
  async function clearCart() {
    await axios.post("https://dogandcat-production.up.railway.app/api/cart/clear", [], {
      headers: { Authorization: `Bearer ${token}` },
    });
  }
  ...
  async function handleAddressData({ target }) {
    ...
    if (name === "pincode" && value.length === 6) {
      const response = await axios.get(
        `https://dogandcat-production.up.railway.appcode.in/pincode/${value}`
      );
  ...
  async function submit(e) {
    ...
    try {
      const response = await axios.post(
        "https://dogandcat-production.up.railway.app/api/address",
        addressData,
