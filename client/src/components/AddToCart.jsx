...
function AddToCart({ isAuthenticated }) {
  ...
  async function fecthingCartData() {
    try {
      const response = await axios.get('http://dogandcat-production.up.railway.app/api/cart', { headers: { Authorization: `Bearer ${token}` } });
  ...
  async function handleCart(event, type, item) {
    ...
    try {
      await axios.post('http://dogandcat-production.up.railway.app/api/cart', updateItems, { headers: { Authorization: `Bearer ${token}` } })
    } catch (error) {
...
async function updateProducts({type,product}) {
  ...
  if( type === 'delete' ){
    if(product._id){
      ...
      const response = await axios.post(`http://dogandcat-production.up.railway.app/api/cart/`,productDetails,{headers:{Authorization:`Bearer ${token}`}});
      window.location.reload()
    }
  }else if(type === 'wishlist' ){
    if(product._id){
      const updateItems = { ...product, updateType:'wishlist' };
      const response = await axios.post('http://dogandcat-production.up.railway.app/api/cart',updateItems,{headers:{Authorization:`Bearer ${token}`}})
      window.location.reload()
    }
  }
}
